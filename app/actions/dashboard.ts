...
type LivePaymentStatus = "PAID" | "PENDING" | "PARTIAL";

function getLivePaymentStatus(totalFee: number, totalPaid: number): LivePaymentStatus {
...
export async function getDashboardData() {
  ...
  const [infos, recordsAgg, passFromAttempts, passFromDrivingResults, upcoming] =
    await Promise.all([
      prisma.paymentInfo.findMany({
        select: {
          applicationId: true,
      ...
      prisma.drivingExamResult.findMany({
        where: { result: "PASS" },
        distinct: ["applicationId"],
        select: { applicationId: true },
      }),
  ...
  const extraMap = new Map<string, number>();
  for (const r of recordsAgg) {
    extraMap.set(r.applicationId, Number(r._sum.amount ?? 0));
  }
  ...
  let totalPaidAmount = 0;         // actual collected money
  let pendingBalanceTotal = 0;     // due for only pending
  let partialBalanceTotal = 0;     // due for only partial
  let totalDueAmount = 0;          // due for pending + partial

  for (const p of infos) {
    const totalFee = Number(p.totalFee ?? 0);
    const advanceFee = Number(p.advanceFee ?? 0);
    const extraPaid = extraMap.get(p.applicationId) ?? 0;

    const totalPaid = advanceFee + extraPaid;
    const due = Math.max(totalFee - totalPaid, 0);

    const liveStatus = getLivePaymentStatus(totalFee, totalPaid);

    totalPaidAmount += Math.min(totalPaid, totalFee);
    totalDueAmount += due;
  ...
  return {
    ...
    paidTotal: totalPaidAmount,      // actual collected money
    pendingBalanceTotal,
    partialBalanceTotal,
    totalToPay: totalDueAmount,      // all due money
